...
def ppo1_nmileg_pool(sensory_value):
	RL_method = "PPO1"
	# total_MC_runs = 50
	experiment_ID = "handtest_rot_pool_with_MC_C_/"
	save_name_extension = RL_method
	total_timesteps =2000000
	sensory_info = "sensory_{}".format(sensory_value) 
	current_mc_run_num = 18# starts from 0
	for mc_cntr in range(current_mc_run_num, current_mc_run_num+1):
		...
		env = gym.make('HandManipulate-v{}'.format(sensory_value))
...
if __name__ == '__main__':
	...
	sensory_versions = 3
	pool.map_async(ppo1_nmileg_pool, [row for row in range(sensory_versions)])
	pool.close()
